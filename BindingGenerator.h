...
void bindKeyboardAndMouse(InputDevice keyboard, InputDevice mouse, std::shared_ptr<PlayerInput> playerInput, int screenHeight) {
...
void bindGamepad(InputDevice gamepad, std::shared_ptr<PlayerInput> playerInput) {
...
void bindGamepadFull(InputDevice gamepad, std::shared_ptr<PlayerInput> playerInput) {
    playerInput->devices.push_back(gamepad);
    InputBinding leftStickX{ .device = gamepad, .axisCode = GLFW_GAMEPAD_AXIS_LEFT_X, .scale = 1.0f, .deadzone = 0.05f };
    InputBinding leftStickY{ .device = gamepad, .axisCode = GLFW_GAMEPAD_AXIS_LEFT_Y, .scale = 1.0f, .deadzone = 0.05f };
    playerInput->bindings[Action::LeftStickX].push_back(leftStickX);
    playerInput->bindings[Action::LeftStickY].push_back(leftStickY);
}